...
sub make_deg_density {
    ...
    if($opt_q) {
	system "bowtie -f -v 1 --best -k 1 --norc -S $opt_n $opt_e 2> /dev/null \| sed -e 's/SO:unsorted/SO:coordinate/' 2> /dev/null \| samtools view -S -b -u - 2> /dev/null \| samtools sort -@ 4 -m 6G - -o $bam_file 2> /dev/null";
    } else {
	system "bowtie -f -v 1 --best -k 1 --norc -S $opt_n $opt_e \| sed -e 's/SO:unsorted/SO:coordinate/' \| samtools view -S -b -u - 2> /dev/null \| samtools sort -@ 4 -m 6G - -o $bam_file 2> /dev/null";
    }
...
#    my $args2 = "$dir_name" . "\/" . "$gfields[0]" . "_" . "$gfields[1]" . "_" . "$gfields[4]" . "_TPlot.pdf";
    my $args2 = "$dir_name" . "\/" . "$gfields[0]" . "_" . "$gfields[1]" . "_" . "$gfields[4]" . "_c" . "$cat_digit" . ".png";
    my $args3 = "$gfields[1]";
    my $args4 = "$gfields[0]";
    my $args5 = "$cat_digit";
    my $args6 = "$gfields[4]";
    ...
    # Call R
    system "R --vanilla --quiet --slave --args $args1 $args2 $args3 $args4 $args5 $args6 $args7 $args8 $args9 < CleaveLand4_Tplotter.R 2> /dev/null";
...
sub write_R_script {
    ...
    print R "average <- sum(data\$All, na.rm=T)/(sum(!is.na(data\$All)))\n";
    print R "cleavage <- na.omit(data)[,c(1,3)]\n";
    print R "tagcount <- sum(data\$Site, na.rm=T)\n";
    print R "query <- paste(\"Query:\", args[4], sep=\"\\t\")\n";
    print R "target <- gsub(\"_\", \" \", args[3])\n";
    print R "target <- sub(\"-\", \".\", target)\n";
    print R "target <- str_wrap(target, width=60, exdent=16)\n";
    print R "main <- paste(query, \"\\n\", \"Target:\", \"\\t\", target, sep=\"\")\n";
    print R "p <- round(as.numeric(args[7]), digits=3)\n";
    print R "c <- as.numeric(args[5])\n";
...
#    print R "pdf(file=args[2])\n";
    ...
    print R "mtext <- paste(\"Category: \", args[5], \" Allen score: \", args[9], \" Cleavage site: \", args[6], \"/\", args[8], \" Tag count: \", tagcount, \" p-value: \", p, sep=\"\")\n";
    print R "mtext(mtext, cex=0.8, adj=0)\n";
    print R "points(data\$Position, data\$Site, pch=15, cex=1.2, col=color)\n";
    print R "points(c(cleavage\$Position, cleavage\$Position), c(0, cleavage\$Site), type=\"l\", lty=2, col=color)\n";
    print R "abline(h=average, col=\"gray60\")\n";
